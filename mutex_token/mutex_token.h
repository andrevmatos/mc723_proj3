...
{
private:
  unsigned volatile int token;
  unsigned int acquire_token();
public:
  /// Exposed port with ArchC interface
  sc_export< ac_tlm_transport_if > target_export;

  ac_tlm_rsp_status get_token( uint32_t &a );
  ac_tlm_rsp_status free_token();
  ...
  */
  ac_tlm_rsp transport( const ac_tlm_req &request ) {
    ...
    if(request.addr == MUTEX_TOKEN_ADDR)
    {
      ...
      {
        response.status = get_token( response.data );
      }
   ...
   *
   */
  mutex_token( sc_module_name module_name);
